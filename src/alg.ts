...
export function useAlgorithm() {
  // Используем общий тип шагов с parentStepId
  ...
  // addStep теперь возвращает свой stepId, чтобы передавать как parentStepId дальше
  ...
  const algorithms = {
    quickSort: (arr: number[], depth = 0, path: number[] = [], parentStepId: number | null = null): number[] => {
      if (arr.length <= 1) return arr;
      const pivotIndex = Math.floor(arr.length / 2);
      const pivot = arr[pivotIndex];
      const leftIndexes = arr
        .map((x, i) => (x < pivot ? i : -1))
        .filter((i) => i !== -1);
      const rightIndexes = arr
        .map((x, i) => (x > pivot ? i : -1))
        .filter((i) => i !== -1);
    ...
    binarySearch: (arr: number[], target: number, depth = 0, path: number[] = [], parentStepId: number | null = null): number => {
      let left = 0;
      ...
      while (left <= right) {
        ...
        const effects: StepEffect[] = [
          { type: "highlight", indices: [mid], label: "mid", color: "bg-red-400" },
          { type: "highlight", indices: [left], label: "left", color: "bg-sky-400" },
          { type: "highlight", indices: [right], label: "right", color: "bg-green-400" },
        ];
        lastStepId = addStep({
    ...
    factorialHash: (n: number, memo: Record<number, number> = {}, depth = 0): number => {
      if (n <= 1) return 1;
      ...
      addStep({
        ...
        depth,
        data: { array: [n], n, value: res, memo: { ...memo } },
        effects: [],
        parentStepId: null,
        path: [],
      });
